<template>
    <div class="activity">
        ...
        <ls-activity-rearrange-name v-if="subtypes.rearrangeName === activity.subtype.slug" :col-sizes="valColSizeObj"></ls-activity-rearrange-name>
    </div>
</template>
<script>
...
export default {
    ...
    computed: {
        ...
        valColSizeObj(){
            return {
                key: Math.abs(TOTAL_COLUMNS / this.activity.item_template.key.total_per_line),
                value: Math.abs(TOTAL_COLUMNS / this.activity.item_template.value.total_per_line)
            }
        },
