    ...
    methods: {
        allowPainting(bool){
            ...
            if(!bool && this.actualWord.length>0){
                let wordStr = this.actualWord.reduce((str,w)=> `${str}${w.value}`, '')
        ...
        createGrid(words){
            ...
            for (let i = 0; i<words.length; i++){
                this.items.push({text: words[i].text, valid: false, id: words[i].id})
        ...
        putWord(word, grid){
            ...
            let check, maxIndex, beginIndex, transversal, tries = 0, maxTries = 10, reversed = false
            while(!check){
                if(tries<maxTries){
                    ...
                    tries++
                    check = this.checkIfFits(grid, beginIndex, transversal, word, dir)
                } else if(!reversed) {
                    if(dir==='hor')
                        dir = 'ver'
                    else if(dir==='ver')
